...
Usage() {
	...
	echo -e "\toptional arguments:\n" \
          ...
          "\t\t-c  (--capture_kit)      = capture kit used for exome targeting (SS38,SS50,SSV4,SSV5,SSV5C,SSV7,NGv3,NGvCRome,TruSeq,Nextera10,Nextera12 or Unknown (CCDS))\n" \
  ...
  if [[ $MODE == "bam" ]]; then
    ...
    if [[ ! `find ./ -maxdepth 1 -name ${DONE}` || $SFS -eq 1 ]]; then
      samToFastq="$QUEUE $ACCOUNT  \
        -lnodes=1:ppn=${THREADS} \
        -lmem=${MAX_MEM}g \
        -lwalltime=6:0:0 \
        -N ${input}-${LIB}-${RUN}.${NAME} \
        -V \
        -v BAM=$(echo $F1|sed 's/_read1.fastq.gz//g') \
        ${PIPELINE_HOME}/data/qsub/${NAME}.qsub"
      if [[ $VERBOSE == 1 ]]; then  echo -e "samToFastq:\n$samToFastq"; fi
      samToFastq=$($samToFastq)
      echo "[Q] SamToFastq      : $samToFastq"
      DEPEND_SAMTOFASTQ="-W "$dependMod"afterok:"$(echo $samToFastq)
    else echo "[D] SamToFastq"
    fi
  fi

#PRE PREPROCESSING
  FASTQC_OUTPUT_DIR=fastqc.${input}-${LIB}-${RUN}
  ...
  if [[ ! `find ./ -maxdepth 1 -name ${DONE}` || $SFS -eq 1 ]]; then
    trimm="$QUEUE $ACCOUNT  \
      ...
      -N ${input}-${LIB}-${RUN}.${NAME} \
      -V \
      -v FASTQ1=$F1,FASTQ2=$F2,TRIM_PAIRED_FASTQ1=$TRIM_PAIRED_FASTQ1,TRIM_PAIRED_FASTQ2=$TRIM_PAIRED_FASTQ2,TRIM_UNPAIRED_FASTQ1=$TRIM_UNPAIRED_FASTQ1,TRIM_UNPAIRED_FASTQ2=$TRIM_UNPAIRED_FASTQ2,FASTQC_OUTPUT_DIR=$FASTQC_OUTPUT_DIR,DONE=$DONE \
      ${PIPELINE_HOME}/data/qsub/${NAME}.qsub"
    if [[ $VERBOSE == 1 ]]; then  echo -e "trimm:\n$trimm"; fi
    trimm=$($trimm)
...
#ALIGN PAIRED READS (bwa mem)
  ...
  DONE=.${input}-${LIB}-${RUN}.${NAME}.v37.done
  echo $DONE >> $EXPECTED_DONE_FILES
  if [[ ! `find ./ -maxdepth 1 -name ${DONE}` || $SFS -eq 1 ]]; then
    v37_alignPE="$QUEUE $ACCOUNT  \
      ...
      -l mem=${MAX_MEM}g \
      -l walltime=$WALLTIME \
      -N ${input}-${LIB}-${RUN}.${NAME}.v37 \
      $DEPEND_TRIM \
      -V \
      -v FASTQ1=$TRIM_PAIRED_FASTQ1,FASTQ2=$TRIM_PAIRED_FASTQ2,SAM=$SAM_V37,LIB=$LIB,RUN=$RUN,GENOME=v37,DONE=$DONE \
      ${PIPELINE_HOME}/data/qsub/${NAME}.qsub"
    if [[ $VERBOSE == 1 ]]; then  echo -e "v37_alignPE:\n$v37_alignPE"; fi
    v37_alignPE=$($v37_alignPE)
...
if [[ $DISCARD_UNPAIRED_READS -eq 0 ]]; then
  ...
  if [[ ! `find ./ -maxdepth 1 -name ${DONE}` || $SFS -eq 1 ]]; then
    v37_alignSE="$QUEUE $ACCOUNT  \
      ...
      $DEPEND_TRIM \
      -V \
      -v FASTQ1=$TRIM_UNPAIRED_FASTQ1,FASTQ2=$TRIM_UNPAIRED_FASTQ2,SAM=$SAM_V37_UNPAIRED,INPUT=${input},LIB=$LIB,RUN=$RUN,GENOME=v37,DONE=$DONE \
      ${PIPELINE_HOME}/data/qsub/${NAME}.qsub"
    if [[ $VERBOSE == 1 ]]; then  echo -e "v37_alignSE:\n$v37_alignSE"; fi
    v37_alignSE=$($v37_alignSE)
...
#PARSING ALIGNMENT (bwa sampe)
  ...
  if [[ ! `find ./ -maxdepth 1 -name ${DONE}` || $SFS -eq 1 ]]; then
    MT_alignPE="$QUEUE $ACCOUNT  \
      ...
      -l mem=${MAX_MEM}g \
      -l walltime=$WALLTIME \
      -N ${input}-${LIB}-${RUN}.${NAME}.MT \
      $DEPEND_TRIM \
      -V \
      -v FASTQ1=$TRIM_PAIRED_FASTQ1,FASTQ2=$TRIM_PAIRED_FASTQ2,SAM=$SAM_MT,LIB=$LIB,RUN=$RUN,GENOME=MT,DONE=$DONE \
      ${PIPELINE_HOME}/data/qsub/${NAME}.qsub"
    if [[ $VERBOSE == 1 ]]; then  echo -e "MT_alignPE:\n$MT_alignPE"; fi
    MT_alignPE=$($MT_alignPE)
...
alignFastq() {
  ...
  if [[ -n $forward && -n $reverse && -n $run && -n $lib ]]; then
    ...
    align $input $forward $reverse $lib $run $V37SAM $MTSAM $V37SAMUNPAIRED $NOMT;
...
alignBam() {
  ...
  if [[ -n $bam && -n $run && -n $lib ]]; then
    ...
    align $input ${bam}_read1.fastq.gz ${bam}_read2.fastq.gz $lib $run $V37SAM $MTSAM $V37SAMUNPAIRED $NOMT;
    if [[ ${DEPEND_V37_ALN} ]]; then ALIGN_V37_IDS=${ALIGN_V37_IDS}${DEPEND_V37_ALN}; fi
    if [[ ${DEPEND_MT_ALN} ]]; then ALIGN_MT_IDS=${ALIGN_MT_IDS}${DEPEND_MT_ALN}; fi
  fi
}
...
if [[ ! `find ./ -maxdepth 1 -name ${DONE}` || $SFS -eq 1 ]]; then
  v37_merge_sample_libs="$QUEUE $ACCOUNT \
    ...
    ${PIPELINE_HOME}/data/qsub/${NAME}.qsub"
  if [[ $VERBOSE == 1 ]]; then  echo -e "v37_merge_sample_libs:\n$v37_merge_sample_libs"; fi
  v37_merge_sample_libs=$($v37_merge_sample_libs)
...
if [[ ! `find ./ -maxdepth 1 -name ${DONE}` || $SFS -eq 1 ]]; then
  gatk_v37="$QUEUE $ACCOUNT  \
    ...
    $V37_MERGE_DEPEND \
    -V \
    -v ALIGNED_BAM=${MERGE_V37_SORT_BAM_PRIMARY_ALN},USE_INDELS=1,GENOME=v37,DONE=${DONE} \
    ${PIPELINE_HOME}/data/qsub/${NAME}.qsub"
  if [[ $VERBOSE == 1 ]]; then  echo -e "gatk_v37:\n$gatk_v37"; fi
  gatk_v37=$($gatk_v37)
...
DONE=.${SAMPLE}.${NAME}.MT.done
echo $DONE >> $EXPECTED_DONE_FILES
MERGE_MT_SORT_BAM=${SAMPLE}.aligned.MT.merged.sorted.bam
if [[ ! -z $ALIGN_MT_IDS ]]; then DEPEND="-W "$dependMod"afterok"`echo ${ALIGN_MT_IDS}`; else unset DEPEND; fi
if [[ ! `find ./ -maxdepth 1 -name ${DONE}` || $SFS -eq 1 ]]; then
  MT_merge_sample_libs="$QUEUE $ACCOUNT  \
    ...
    $DEPEND \
    -V \
    -v MERGE_SORT_SAMS=${MERGE_MT_SORT_SAMS},MERGE_SORT_BAM=${MERGE_MT_SORT_BAM},DONE=${DONE} \
    ${PIPELINE_HOME}/data/qsub/${NAME}.qsub"
  if [[ $VERBOSE == 1 ]]; then  echo -e "MT_merge_sample_libs:\n$MT_merge_sample_libs"; fi
  MT_merge_sample_libs=$($MT_merge_sample_libs)
  echo "[Q] Merge Sort MT   : $MT_merge_sample_libs [$DEPEND]"
  MT_MERGE_DEPEND="-W "$dependMod"afterok:"`echo $MT_merge_sample_libs`;
else
...
if [[ ! `find ./ -maxdepth 1 -name ${DONE}` || $SFS -eq 1 ]]; then
  gatk_MT="$QUEUE $ACCOUNT  \
    ...
    $MT_MERGE_DEPEND \
    -V \
    -v ALIGNED_BAM=${MERGE_MT_SORT_BAM},USE_INDELS=0,GENOME=MT,DONE=${DONE} \
    ${PIPELINE_HOME}/data/qsub/${NAME}.qsub"
  if [[ $VERBOSE == 1 ]]; then  echo -e "gatk_MT:\n$gatk_MT"; fi
  gatk_MT=$($gatk_MT)
  echo "[Q] GATK MT         : $gatk_MT [$MT_MERGE_DEPEND]"
  GATK_MT_DEPEND="-W "$dependMod"afterok:"`echo $gatk_MT`;
else echo "[D] GATK MT"
fi
...
if [[ ! `find ./ -maxdepth 1 -name "$DONE"` || $SFS -eq 1  ]]; then
  sample_filter_MT="$QUEUE $ACCOUNT  \
    ...
    -v DONE=${DONE} \
    ${PIPELINE_HOME}/data/qsub/${NAME}.qsub"
  if [[ $VERBOSE == 1 ]]; then  echo -e "sample_filter_MT:\n$sample_filter_MT"; fi
  sample_filter_MT=$($sample_filter_MT)
  echo "[Q] Filter MT       : $sample_filter_MT [$GATK_MT_DEPEND]"
  FILTER_MT_DEPEND="-W "$dependMod"afterok:"`echo $sample_filter_MT`;
...
if [[ ! `find ./ -maxdepth 1 -name ${DONE}` || $SFS -eq 1 ]]; then
  MT_sample_stats_QC="$QUEUE $ACCOUNT  \
    ...
    ${FILTER_MT_DEPEND} \
    -V \
    -v CAPTURE_KIT=${CAPTURE_KIT},DONE=${DONE} \
    ${PIPELINE_HOME}/data/qsub/${NAME}.qsub"
  if [[ $VERBOSE == 1 ]]; then  echo -e "MT_sample_stats_QC:\n$MT_sample_stats_QC"; fi
  MT_sample_stats_QC=$($MT_sample_stats_QC)
  echo "[Q] MT Stats and QC : $MT_sample_stats_QC [$FILTER_MT_DEPEND]"
  MT_STATS_QC_DEPEND="-W "$dependMod"afterok:"`echo $MT_sample_stats_QC`;
...
if [[ ! `find ./ -maxdepth 1 -name ${DONE}` || $SFS -eq 1 ]]; then
  Exome_sample_stats_QC="$QUEUE $ACCOUNT  \
    ...
    -v CAPTURE_KIT=${CAPTURE_KIT},DONE=${DONE} \
    ${PIPELINE_HOME}/data/qsub/${NAME}.qsub"
  if [[ $VERBOSE == 1 ]]; then  echo -e "Exome_sample_stats_QC:\n$Exome_sample_stats_QC"; fi
  Exome_sample_stats_QC=$($Exome_sample_stats_QC)
...
#    ${PIPELINE_HOME}/data/qsub/${NAME}.qsub"
#  if [[ $VERBOSE == 1 ]]; then  echo -e "gatk_UG_total_recall:\n$gatk_UG_total_recall"; fi
#  gatk_UG_total_recall=$($gatk_UG_total_recall)
...
if [[ ! `find ./ -maxdepth 1 -name ${DONE}` || $SFS -eq 1 ]]; then
  gatk_HC="$QUEUE $ACCOUNT  \
    ...
    -v ${CALLING_BED_ARG}DONE=${DONE} \
    ${PIPELINE_HOME}/data/qsub/${NAME}.qsub"
  if [[ $VERBOSE == 1 ]]; then  echo -e "gatk_HC:\n$gatk_HC"; fi
  gatk_HC=$($gatk_HC)
