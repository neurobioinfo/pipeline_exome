...
if ($vcfinput) {
	...
	print STDERR "\nNOTICE: Running with system command <$sc>\n";
	system ($sc) and die "Error running system command: <$sc>\n";
	push @unlink, "$tempfile.${buildver}_multianno.txt";
...
sub filterOperation {
	...
	if ($dbtype =~ m/^ljb\d*/ or $dbtype =~ m/^popfreq/ or $dbtype =~ m/^custom/) {
		$sc .= " -otherinfo";
	...
	while (<FH>) 
	{
		m/^([^\t]+)\t([^\t]+)\t(\S+\s+\S+\s+\S+\s+\S+\s+\S+).*/ or die "Error: invalid record found in annovar outputfile: <$_>\n";
		my ($db,$anno,$varstring)=($1,$2,$3);
		#$anno =~ s/\\x2c/,/g;			#when annotating filter database with multiple annotations, the comma is replaced by \x23, when -otherinfo is used #commented out, we cannot do it here, otherwise extra columns are printed
